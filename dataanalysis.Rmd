...
```{r, warning=FALSE}
library(readr)
df <- read_csv("~/data/gcme/data_received_190301/NewData_wide_CORRECTED.csv")
```
...
```{r}
long_to_wide_gcme <- function(df_long){

  joinvars <- names(df_long)[-which(names(df_long) %in% c("level", "mean", "sd", "se", "c"))]
  factors_all <- df_long %>%
    filter(!(factors %in% c("x","x99"))) %>%      # Something weird with this
    select(factors) %>% 
    unique() %>% 
    ...
    unname() %>% 
    unique()
  factorvars <- factors_all[-which(factors_all=="c")]
    
  # take all data where factor 'c' is TRUE
  df_wide_c <- dplyr::filter(df_long, c) %>% 
    ...
    left_join( 
      ...
      dplyr::filter(df_long, !c) %>% 

        # # remove 'ambients' where 'c' is not element of treatment - we don't need that 
        # filter( !(!grepl("c", treatment) & level=="ambient") ) %>% 
    ...
    # order columns
    select( c(exp_nam, factors, factorvars, ambient, elevated, ambient_se, elevated_se, ambient_sd, elevated_sd)  )
