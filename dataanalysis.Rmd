...
Add `my_varnam` to the data table, defined by `my_varnam = ifelse( varnam %in% c("litter_prod", "litter_mass"))`.
```{r, warning=FALSE}
df_c <- df_c %>%
...
df_varnams <- tibble(
  my_varnam = c("my_anpp", "my_nmass_leaf", "my_narea_leaf", "my_asat", "my_anet", "my_bnpp", "my_navl",
                "my_rootshootratio", "my_nup",   "my_nleach",  "my_n2o",        "my_kdecay_soil", "my_litter"),
  my_lab    = c("ANPP",    "Leaf Nmass",    "Leaf Narea",    "Asat",    "Anet",    "BNPP",    "N availability",
                "root:shoot ratio",  "N uptake", "N leaching", "N2O emissions", "Soil decomposition rate", "Litter")
)
```
...
df_c_sub <- df_c %>%         
  ...
  filter(my_varnam %in% c("my_anpp", "my_nmass_leaf", "my_narea_leaf", "my_asat", "my_bnpp", "my_navl", "my_rootshootratio", "my_nup", "my_kdecay_soil", "my_litter")) %>%
