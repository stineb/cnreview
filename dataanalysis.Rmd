...
df_varnams <- tibble(
  ...
  my_lab    = c("ANPP",    "Nmass",         "Narea",         "Asat",    "Anet",    "BNPP",    "Navl")
)
```
...
df_c_sub <- df_c %>%         
  ...
  ## Here only for my variables, selected as described above (leaving out "my_anet")
  filter(my_varnam %in% c("my_anpp", "my_nmass_leaf", "my_narea_leaf", "my_asat", "my_bnpp", "my_navl")) %>%
...
df_varnams_sub <- df_varnams %>% filter(my_varnam!="my_anet")
purrr::map(
